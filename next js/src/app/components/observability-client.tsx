...
export default function ObservabilityClient({ initialData }: Props) {
  const base: Deployment[] = Array.isArray(initialData) ? initialData : [];
  ...
  const filtered = useMemo(() => {
    ...
    const rangeMs =
      ...
      range === "30d" ? 30 * 24 * 60 * 60 * 1000 : Infinity;
    ...
    const rows = base.filter((d) => {
      ...
      const matchesEnv = env === "All" || d.env.toLowerCase() === env.toLowerCase();
      const matchesCloud = cloud === "All" || d.cloud.toLowerCase() === cloud.toLowerCase();
  ...
  }, [base, search, env, cloud, status, range, sort]);
  ...
  <FilterDropdown label="Env"   value={env}   onChange={setEnv}   options={ENVS} />
  <FilterDropdown label="Cloud" value={cloud} onChange={setCloud} options={CLOUDS} />
  <FilterDropdown label="Status" value={status} onChange={setStatus} options={STATUSES} />
  <FilterDropdown label="Range" value={range} onChange={setRange} options={RANGES} />
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <div className="text-sm text-gray-700">
                      {d.cloud} <span className="text-gray-400">•</span> {d.region}
                    </div>
                  </td>
